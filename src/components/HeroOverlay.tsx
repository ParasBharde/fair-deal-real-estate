    ...
    <div className="pointer-events-none absolute inset-0 z-20">
      <div className="mx-auto flex h-full max-w-7xl flex-col justify-between px-4 py-5 md:px-8 md:py-8">
        ...
        <div className="grid items-end gap-8 pb-14 pt-10 md:pb-16 md:pt-14 lg:grid-cols-[1.08fr_0.92fr] lg:pt-20">
          ...
          >
            <div className="rounded-[2rem] border border-glass-border-light bg-[linear-gradient(180deg,rgba(var(--color-background),0.38),rgba(var(--color-background),0.16))] p-5 backdrop-blur-[3px] md:p-7">
              <p className="mb-4 text-xs uppercase tracking-[0.42em] text-primary/90">
                FAIR DEAL REAL ESTATE
              </p>
              <h1 className="text-4xl leading-[0.9] text-heading sm:text-5xl md:text-[4.3rem] xl:text-[4.9rem]">
                PCMC real estate,
              ...
              <p className="mt-5 max-w-xl text-sm leading-7 text-muted-text-less md:text-base">
            ...
            <div className="mt-6 flex flex-wrap gap-3 text-xs uppercase tracking-[0.24em] text-muted-text-less">
              {promisePoints.slice(0, 3).map((item) => (
                <span
                  key={item.id}
                  className="rounded-full border border-glass-border bg-glass px-4 py-2 backdrop-blur-md"
                >
                  {item.title}
                </span>
              ))}
            ...
            <div className="mt-8 grid max-w-3xl gap-3 sm:grid-cols-3">
              {highlights.map((item) => (
                <div key={item.id} className="glass-panel rounded-3xl p-4">
                  ...
                  <p className="mt-1 text-xs leading-5 text-muted-text">{item.label}</p>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          >
            ...
            >
              ...
              <div className="rounded-[1.5rem] border border-primary/20 bg-dark-panel p-3">
                <div className="flex items-center gap-3 rounded-[1.2rem] border border-glass-border-light bg-glass px-4 py-3">
                  ...
                  <input
                    ...
                    className="w-full bg-transparent text-sm text-heading placeholder:text-muted-text-more focus:outline-none"
                  />
                </div>
                ...
                <div className="mt-4 rounded-[1.1rem] border border-glass-border-light bg-dark-panel px-4 py-3">
                  <div className="mb-2 flex items-center gap-2 text-[10px] uppercase tracking-[0.24em] text-primary/85">
                    <Sparkles size={12} />
                    Active map response
                  </div>
                  <p className="text-xs leading-6 text-muted-text">{searchMessage}</p>
                </div>
              </div>
            </form>

            <div className="mt-5 flex items-center gap-2 text-[11px] uppercase tracking-[0.24em] text-muted-text-more">
