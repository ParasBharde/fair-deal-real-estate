  ...
  return (
    <section className="relative overflow-hidden border-b border-gold/12 bg-section-glow px-4 pb-10 pt-4 md:px-8 md:pb-12 md:pt-6">
      <div className="mx-auto max-w-7xl">
        ...
        <div
          id="live-map"
          className="mt-8 overflow-hidden rounded-[2rem] border border-gold/16 bg-[linear-gradient(180deg,rgba(18,18,18,0.96),rgba(8,8,8,1))] shadow-panel md:mt-10"
        >
          <div className="flex items-center justify-between border-b border-white/8 px-5 py-4">
            <div>
              <p className="text-[11px] uppercase tracking-[0.26em] text-gold/85">Live PCMC Map</p>
              <p className="mt-1 text-sm text-stone-400">
                Area chips stay on-map. The selected property dock stays pinned at the edge.
              </p>
            </div>
            <div className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-[10px] uppercase tracking-[0.24em] text-stone-300">
              Pin explore
            </div>
          </div>

          <div className="relative">
            <div ref={mapContainerRef} className="h-[420px] w-full touch-pan-x touch-pan-y md:h-[500px]" />

            <div className="pointer-events-none absolute inset-x-0 top-0 h-28 bg-[linear-gradient(180deg,rgba(0,0,0,0.62),transparent)]" />
            <div className="pointer-events-none absolute inset-x-0 bottom-0 h-36 bg-[linear-gradient(180deg,transparent,rgba(0,0,0,0.84))]" />

            <div className="absolute left-4 right-4 top-4 z-10 flex flex-wrap gap-2 md:left-5 md:right-28">
              {neighborhoods.map((area) => (
                <button
                  key={area.id}
                  type="button"
                  onClick={() => handleAreaFilter(area)}
                  className={`rounded-full border px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.22em] backdrop-blur-md transition ${
                    activeAreaFilter === area.name
                      ? "border-gold bg-gold/18 text-gold"
                      : "border-white/16 bg-black/30 text-stone-200 hover:border-gold/30 hover:text-gold"
                  }`}
                >
                  {area.name}
                </button>
            ...
            <div className="absolute bottom-4 left-4 right-4 z-10 md:bottom-5 md:left-5 md:max-w-[430px]">
              <div className="rounded-[1.6rem] border border-gold/20 bg-[linear-gradient(180deg,rgba(18,18,18,0.9),rgba(10,10,10,0.94))] p-4 shadow-panel backdrop-blur-xl">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.24em] text-gold/85">
                      <Compass size={13} />
                      Selected on map
                    </div>
                    <h3 className="mt-2 text-2xl text-sand">{selectedProperty.title}</h3>
                    <p className="mt-1 text-sm font-semibold text-white">
                      {selectedProperty.configuration} - {selectedProperty.price}
                    </p>
                  </div>
                  <div className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-[10px] uppercase tracking-[0.22em] text-stone-300">
                    {selectedProperty.area}
                  </div>
                </div>

                <p className="mt-3 text-sm leading-6 text-stone-300">{mapMessage}</p>

                <div className="mt-4 flex flex-wrap gap-3">
                  <Link
                    to={`/projects/${selectedProperty.id}`}
                    className="rounded-full border border-gold/40 bg-gold/15 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.22em] text-gold transition hover:bg-gold/25"
                  >
                    View Property
                  </Link>
                  <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-[10px] uppercase tracking-[0.22em] text-stone-300">
                    <BedDouble size={12} />
